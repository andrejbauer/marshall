(* \section{Abstract syntax (module [Syntax])} *)
...
module Syntax = 
  functor (D: Dyadic.DYADIC) ->
    struct
      ...
      (* The type of variable names. Variables are either `original' or
	generated. *)
      type name =
	| Ident of string
	| Gensym of int
	...
	| Gensym k -> "gen" ^ string_of_int k
      ...
      (* In Marshall we have base types for reals and propositions, product
	types and function types. Function types are a mirage because
	all $\lambda$-abstractions get $\beta$-reduced away. *)
